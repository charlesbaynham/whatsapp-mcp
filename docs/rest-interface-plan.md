...
```
                 ┌──────────────────────────────┐
  WhatsApp ◄────►│  whatsapp-bridge (Go)         │  127.0.0.1:8080 (or unix socket)
                 │  - whatsmeow session          │
...
2. Clients talk HTTP only. A shared filesystem is *not* required for
   text; media paths are optional convenience for co-located clients.
3. Two push mechanisms with different guarantees:
   ...
   - **Webhooks** (existing): outbound POST, lossy by design, rate-capped.
     For Claude routines and other remote targets.
...
Keep `127.0.0.1` as the default. Add `unix:/path.sock` support for
`WHATSAPP_BRIDGE_ADDR` so the nix module can hand each client unit a socket
with file permissions instead of relying on loopback plus same-user. Small
change (`net.Listen("unix", ...)` plus `http.Server.Serve`).

## Client side

### `whatsapp-client` (Python package, in-repo)

Thin `httpx` wrapper: one method per endpoint, dataclasses matching the JSON,
...
usage. `media_path` for `send_file` still refers to a store path (same
container) but the containment check lives only in Go.
...
and a `services.whatsapp.clients.<name>` option set. Client units get the
same hardening, `After=whatsapp-bridge.service`, and either loopback or the
socket path. Only the MCP unit is reachable from outside the container;
forwarder units have no listening port at all.

## Phasing

1. **Read API in Go** with table tests. Additive, no client change.
...
4. **Voice notes and media streaming in Go**.
5. **hindsight-forwarder** plus nix unit.
6. Optional: Unix socket binding.
