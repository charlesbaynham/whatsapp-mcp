...
| GET | `/send/{id}` | | How a submission went: `{id, state, success, message, queued_at, sent_at}`, `state` one of `queued`, `sent`, `failed`. Only recent sends are kept; `404` once one ages out. |
